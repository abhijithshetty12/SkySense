...
export function Dashboard() {
  ...
  const generateTabs = (
    activeTab: "overview" | "forecast" | "ai", 
    setActiveTab: React.Dispatch<React.SetStateAction<"overview" | "forecast" | "ai">>, 
    tabValues: ("overview" | "forecast" | "ai")[], 
    tabLabels: string[], 
    tabStyles: string[]
  ) => {
    return tabValues.map((value, index) => (
      <TabsTrigger 
        key={value}
        value={value} 
        className={`h-9 sm:h-10 md:h-auto flex-1 py-1.5 text-xs sm:text-sm data-[state=active]:shadow-[0_0_0_2px_var(--primary)] ${activeTab === value ? "bg-primary border-primary shadow-primary" : ""} ${tabStyles[index]}`}
      >
        {tabLabels[index]}
      </TabsTrigger>
    ));
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--background)" }}>
      ...
      <header
        className="sticky top-0 z-40 flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-3 sm:px-6 sm:py-2.5 overflow-visible"
        style={{ 
          background: "var(--card)", 
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-6 space-y-6 sm:space-y-7">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList 
              className="w-full grid grid-cols-3 gap-1 p-1 rounded-md" 
              style={{ background: "var(--card)", backdropFilter: "blur(16px)", border: "1px solid var(--border)" }}
            >
